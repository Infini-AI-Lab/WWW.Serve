...
class TestCreditLedger:
    ...
    async def create_account(self, node_id: str) -> bool:
        async with self.accounts_lock:
            if node_id not in self.accounts:
                self.accounts[node_id] = CreditAccount(node_id=node_id)
    ...
    def select_node_by_pos(self, self_node_id: str, seed: str, k = 5) -> List[str]:
