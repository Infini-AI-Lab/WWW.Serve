...
async def node_offline(node: LLMNode, delay=0):
    ...
    await node.stop()
    print(f"Node {node.node_id} is now offline.")
...
async def main():
    ...
    tasks = [asyncio.create_task(timed_submit(item["problem"], node1)) for idx, item in enumerate(data)]

    asyncio.create_task(node_offline(node2, 10))
